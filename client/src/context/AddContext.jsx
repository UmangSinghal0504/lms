...
export const AppContextProvider = (props) => {
    ...
    const constructUrl = (basePath, id) => {
        if(!basePath || !id) {
            throw new Error("Base path and ID are requried to construct URL.")
        }
        return `${basePath}/${id}`;
    }

    // Function to navigate to a contructed URL
    const navigateToCourse = (id) => {
        const url = constructUrl("/course",id);
        navigate(url);
    }
    ...
    const value= {
        currency,allCourses,navigate,calculateRating,isEducator, setIsEducator,constructUrl,navigateToCourse,
